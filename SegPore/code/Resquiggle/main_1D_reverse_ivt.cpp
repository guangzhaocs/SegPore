...
void run_one_read(vector<float>& mu_arr, vector<float>& sigma_arr, vector<int>& label_arr,  vector<int>& start_pos_arr,
    ...
    // do not reverse the current signal, and add -1 before the arr
    for(int i=0;i<n_mu;i++){  
        mu_val_arr[i+1] = mu_arr[i];
        sigma_val_arr[i+1] = sigma_arr[i];
        start_pos_val_arr[i+1] = start_pos_arr[i];
        end_pos_val_arr[i+1] = end_pos_arr[i];
        len_pos_val_arr[i+1] = len_arr[i];
    }
    ...
    vector<string> kmer_str_arr_orgin;
    vector<int> kmer_pos_arr_orgin;
    label2seq(label_arr, contig_pos_start, kmer_str_arr_orgin, kmer_pos_arr_orgin);
    assert(kmer_str_arr_orgin.size() == kmer_pos_arr_orgin.size());
	int n_kmer = static_cast<int>(kmer_str_arr_orgin.size());
	
	// reverse the kmer arr
	vector<string> kmer_str_arr;
    vector<int> kmer_pos_arr;
	kmer_str_arr.push_back("-");
    kmer_pos_arr.push_back(-1);
    for(int i=n_kmer-1;i>=0;i--){
		kmer_str_arr.push_back(kmer_str_arr_orgin[i]);
        kmer_pos_arr.push_back(kmer_pos_arr_orgin[i]);
    }
