...
float log_score(map<string, vector<float> >& ref_kmer_dict, float mu_val, string kmer){
    ...
    static const float unif_log_prob = -4.941642;
    ...
    if(std::abs(mu_val) < 1e-10 || kmer == "-"){
        ...
        float kmer_mu2 = ref_kmer_dict[kmer][2];
        float kmer_sigma2 = ref_kmer_dict[kmer][3];

        if(std::abs(kmer_mu2) < 1e-10){
            float a = (mu_val - kmer_mu1) / kmer_sigma1;
            return std::log(inv_sqrt_2pi / kmer_sigma1 * std::exp(-0.5f * a * a));
        }else{
            float a1 = (mu_val - kmer_mu1) / kmer_sigma1;
            float res1 = std::log(inv_sqrt_2pi / kmer_sigma1 * std::exp(-0.5f * a1 * a1));

            float a2 = (mu_val - kmer_mu2) / kmer_sigma2;
            float res2 = std::log(inv_sqrt_2pi / kmer_sigma2 * std::exp(-0.5f * a2 * a2));
...
void run_one_read(vector<float>& mu_arr, vector<float>& sigma_arr, vector<int>& label_arr,  vector<int>& start_pos_arr,
                  vector<int>& end_pos_arr, vector<int>& len_arr,  vector<string>& read_info_arr, map<string, vector<float> >& ref_kmer_dict,
    ...
    int contig_pos_start = stoi(read_info_arr[3]);
    //int contig_pos_end = stoi(read_info_arr[4]);
...
int main(int argc, char ** argv)
{
    ...
    string ref_kmer_dir = ref_kmer_file;
