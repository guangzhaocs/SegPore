...
//    vector<SortWindow>().swap(sort_win_arr);
    ...
    free(signal_arr_h);
    free(b_st_sig_ind_arr_h);
    free(b_en_sig_ind_arr_h);
    free(B_mu_mat_h);
    free(B_sigma_mat_h);
    free(B_prev_mu_mat_h);
    free(B_prev_sigma_mat_h);
    free(B_next_mu_mat_h);
    free(B_next_sigma_mat_h);
    free(B_hmm_loglik_arr_h);

    signal_arr_h = nullptr;
    b_st_sig_ind_arr_h = nullptr;
    b_en_sig_ind_arr_h = nullptr;
    B_mu_mat_h = nullptr;
    B_sigma_mat_h = nullptr;
    B_prev_mu_mat_h = nullptr;
    B_prev_sigma_mat_h = nullptr;
    B_next_mu_mat_h = nullptr;
    B_next_sigma_mat_h = nullptr;
    B_hmm_loglik_arr_h = nullptr;
}
...
void outer_hmm(int debug, vector<float>& signal_arr, vector<int>& border_arr,
               vector<int>& final_border_arr, vector<int>& final_state_arr){
...
int main(int argc, char ** argv)
{
    ...
    vector<float> signal_arr;
    vector<int> border_arr;
    readOnefrom2DFloatFile(signal_dir, signal_arr, idx);
    readOnefrom2DIntFile(border_dir, border_arr, idx);

    int M = signal_arr.size();
    int N = border_arr.size();
    if (M==0){return 0;}
    ...
    try {
        if(N%2 == 0){
            outer_hmm(debug, signal_arr, border_arr, final_border_arr, final_state_arr);
               cout << " * Read " << idx << " finished ! " << endl;
        }else{
              throw(N);
        }
    }catch(int border_len) {
        cout << " * Read " << idx << " failed !  Border length must be even !"<< endl;
    }


    write_final_int_res_to_csv(save_border_dir, final_border_arr, idx);
    write_final_int_res_to_csv(save_state_dir, final_state_arr, idx);
    return 0;
}
